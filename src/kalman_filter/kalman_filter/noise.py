...
class Noise(Node):
    ...
    def Noise(self,msg:Odometry):
        self.nosied_Reading=msg
        #add gaussian noise to the ground truth reading
        self.nosied_Reading.pose.pose.position.x=self.nosied_Reading.pose.pose.position.x+np.random.normal(0,0.1)
        self.nosied_Reading.pose.pose.position.y=self.nosied_Reading.pose.pose.position.y+np.random.normal(0,0.1)
